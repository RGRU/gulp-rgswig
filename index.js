...
var findCrossData = function (dirPath) {
  ...
  // #1 Find setup
  // ПОЧЕМУ-ТО как глобальная переменная не был объявлен,
  // если все поломается то из-за этого
  // !!!!!! ОБРАТИ ВНИМАНИЕ СЮДА ПРИ ОШИБКЕ !!!!!!!
...
module.exports = function (userOptions) {
  ...
  var rgswig = function (file, callback) {
    ...
    // Compile type
    var compileType = options.param.compileType

    // Ext file
    var extFile = options.param.extFile
    ...
    // Template data
    var tmplData = (options.data) ? options.data : null

    // Template cross data
    var tmplCrossData = null
    ...
    var customHtmlMinify = (options.customHtmlMinify) ? true : false
